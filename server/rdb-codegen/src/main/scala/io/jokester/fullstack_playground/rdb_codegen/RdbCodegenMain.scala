...
object RdbCodegenMain extends App with LazyLogging {
  ...
  val destDir = "api-server/src/main/scala/" + destPkg.split("\\.").mkString("/")
  ...
  MoreFiles.deleteRecursively(Paths.get(destDir))

  val allComplete = Future.sequence(traitsCodeGen.writeFiles(location = destDir))
  Await.result(allComplete, scala.concurrent.duration.Duration.Inf)
  logger.info("DONE")
}
