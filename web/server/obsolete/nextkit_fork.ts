...
export type GetAPIContext<T> = T extends (handlers: Record<Method, NextkitHandler<infer Context, unknown>>) => unknown
  ? Context
  : never;
...
export default function createAPI<Context = null>(config: Config<Context>) {
  ...
  const handler =
    <Init extends HandlerInit, Handlers extends HandlersMap<Context, Init> = HandlersMap<Context, Init>>(
      handlers: Handlers,
